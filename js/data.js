...
const VLSIData = {
  ...
  topics: {
    ...
    "uvm": {
      ...
      interviews: [{ q: "UVM Phase Order?", a: "build_phase executes top-down; connect and elaboration phases run bottom-up; run_phase executes in parallel as a time-consuming task phase." }]
    },
    "dft": {
...
const mainProblemSeeds = [
  ...
  { title: "Four Wires", topic: "Verilog Basics", difficulty: "Easy", desc: "Wire 4 inputs (a, b, c, d) into 4 outputs (w, x, y, z).", code: "assign w = a; assign x = b; assign y = c; assign z = d;" },
  ...
  { title: "8-to-3 Encoder", topic: "Combinational Logic", difficulty: "Medium", desc: "Implement an 8-to-3 line standard binary encoder for one-hot inputs.", code: "always @(*) begin case(in) 8'h01: out=3'd0; 8'h02: out=3'd1; 8'h04: out=3'd2; 8'h08: out=3'd3; 8'h10: out=3'd4; 8'h20: out=3'd5; 8'h40: out=3'd6; 8'h80: out=3'd7; default: out=3'd0; endcase end" },
  ...
  { title: "4-bit Carry Lookahead Adder", topic: "Combinational Logic", difficulty: "Hard", desc: "Design a 4-bit Lookahead Adder generating fast carry bits.", code: "assign p = a ^ b; assign g = a & b; assign c[1] = g[0] | (p[0] & cin); assign c[2] = g[1] | (p[1] & g[0]) | (p[1] & p[0] & cin); assign c[3] = g[2] | (p[2] & g[1]) | (p[2] & p[1] & g[0]) | (p[2] & p[1] & p[0] & cin); assign c[4] = g[3] | (p[3] & c[3]); assign sum = p ^ c[3:0]; assign cout = c[4];" },
  ...
  { title: "7-Segment Display Decoder", topic: "Combinational Logic", difficulty: "Medium", desc: "Map 4-bit BCD input to 7-segment cathode signals (a-g).", code: "always @(*) begin case(bcd) 4'd0: seg=7'b1111110; 4'd1: seg=7'b0110000; 4'd2: seg=7'b1101101; 4'd3: seg=7'b1111001; 4'd4: seg=7'b0110011; 4'd5: seg=7'b1011011; 4'd6: seg=7'b1011111; 4'd7: seg=7'b1110000; 4'd8: seg=7'b1111111; 4'd9: seg=7'b1111011; default: seg=7'b0; endcase end" },
  ...
  { title: "4-bit Arithmetic Logic Unit (ALU)", topic: "Combinational Logic", difficulty: "Hard", desc: "Design a 4-bit ALU supporting ADD, SUB, AND, OR, XOR, NOT operations.", code: "always @(*) begin case(op) 3'd0: out=a+b; 3'd1: out=a-b; 3'd2: out=a&b; 3'd3: out=a|b; 3'd4: out=a^b; 3'd5: out=~a; default: out=4'b0; endcase end" },
  ...
  { title: "Leading Zero Counter", topic: "Combinational Logic", difficulty: "Hard", desc: "Count the number of leading zeros in an 8-bit vector.", code: "always @(*) begin if(in[7]) out=3'd0; else if(in[6]) out=3'd1; else if(in[5]) out=3'd2; else if(in[4]) out=3'd3; else if(in[3]) out=3'd4; else if(in[2]) out=3'd5; else if(in[1]) out=3'd6; else if(in[0]) out=3'd7; else out=4'd8; end" },
